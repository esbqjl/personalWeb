...
import json
import math
from static.deep_learning.python.classification.base.base_predictor import BasePredictor
from static.deep_learning.python.classification.bert_fc.bert_fc_model import BertFCModel
from static.deep_learning.python.classification.vocab import Vocab
...
class BertFCPredictor(BasePredictor):
    ...
    def predict(self,texts,batch_size=64, max_len=512):
        ...
        for batch_idx in range(math.ceil(len(texts)/batch_size)):
            ...
            with torch.no_grad():
                best_labels, logits = self.model(batch_input_ids,batch_att_mask)
